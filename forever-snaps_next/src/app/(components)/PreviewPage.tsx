...
const PreviewPage = ({ previewUrl, uploading, confirmPreviewUpload, cancelPreview }: { previewUrl: string | null; uploading: boolean; confirmPreviewUpload: () => void; cancelPreview: () => void }) => {
  return (
    <main className="min-h-screen bg-white p-6 flex flex-col items-center justify-center font-sans text-gray-700">
      <div className="bg-white backdrop-blur-md p-6 rounded-3xl shadow-xl border border-gray-300 max-w-sm w-full flex flex-col items-center">
        <h2 className="text-2xl font-serif font-bold mb-4">{DICTIONARY.ES.PREVIEW_PAGE.TITLE}</h2>
        <p>{DICTIONARY.ES.PREVIEW_PAGE.DESCRIPTION}</p>
        ...
        <div className="flex flex-col w-full gap-4">
          ...
          >
            {uploading ? DICTIONARY.ES.PREVIEW_PAGE.UPLOADING : DICTIONARY.ES.PREVIEW_PAGE.CONFIRM}
          </button>
          <button
          ...
          >
            {DICTIONARY.ES.PREVIEW_PAGE.RETAKE}
          </button>
        </div>
      </div>
    </main>
